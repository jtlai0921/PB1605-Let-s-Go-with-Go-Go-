...
type OrderedMap interface {
	GenericMap // 泛化的Map接口
	// 取得第一個鍵值。若無任何鍵值對則傳回nil。
	FirstKey() interface{}
	// 取得最後一個鍵值。若無任何鍵值對則傳回nil。
	LastKey() interface{}
	// 取得由小於鍵值toKey的鍵值所對應的鍵值對群組成的OrderedMap型態值。
	HeadMap(toKey interface{}) OrderedMap
	// 取得由小於鍵值toKey且大於等於鍵值fromKey的鍵值所對應的鍵值對群組成的OrderedMap型態值。
	SubMap(fromKey interface{}, toKey interface{}) OrderedMap
	// 取得由大於等於鍵值fromKey的鍵值所對應的鍵值對群組成的OrderedMap型態值。
