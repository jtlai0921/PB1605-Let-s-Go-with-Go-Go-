...
type myGenerator struct {
	caller      lib.Caller           // 呼叫器。
	timeoutNs   time.Duration        // 處理逾時時間，單位：毫微秒。
	lps         uint32               // 每秒酬載量。
	durationNs  time.Duration        // 負載持續時間，單位：毫微秒。
	concurrency uint32               // 平行量。
	tickets     lib.GoTickets        // Goroutine票池。
	stopSign    chan byte            // 停止訊號的傳遞通道。
	cancelSign  byte                 // 取消傳送後續結果的訊號。
	endSign     chan uint64          // 完結訊號的傳遞通道，同時被用於傳遞呼叫執行計數。
	callCount   uint64               // 呼叫執行計數。
	status      lib.GenStatus        // 狀態。
	resultCh    chan *lib.CallResult // 呼叫結果通道。
}
