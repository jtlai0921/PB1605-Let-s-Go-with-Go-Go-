...
const (
	CHANNEL_MANAGER_STATUS_UNINITIALIZED ChannelManagerStatus = 0 // 未起始化狀態。
	CHANNEL_MANAGER_STATUS_INITIALIZED   ChannelManagerStatus = 1 // 已起始化狀態。
	CHANNEL_MANAGER_STATUS_CLOSED        ChannelManagerStatus = 2 // 已關閉狀態。
)

// 表示狀態程式碼與狀態名稱之間的映射關系的字典。
...
type myChannelManager struct {
	...
	reqCh       chan base.Request    // 請求通道。
	respCh      chan base.Response   // 響應通道。
	itemCh      chan base.Item       // 項目通道。
	errorCh     chan error           // 錯誤通道。
	status      ChannelManagerStatus // 通道管理器的狀態。
...
// 檢查狀態。在取得通道的時候，通道管理器應處於已起始化狀態。
// 若果通道管理器未處於已起始化狀態，那麼本方法將會傳回一個非nil的錯誤值。
